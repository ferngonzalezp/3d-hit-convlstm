...
class convttlstm(LightningModule):
  ...
  def test_step(self, batch, batch_idx):
    ...
    loss = torch.sqrt(F.mse_loss(pred, origin, reduction='sum')/torch.sum(origin**2))
  ...
  def test_epoch_end(self, outputs):
    ...
    fig = plt.figure()
    plt.subplot(2,3,1)
    plt.imshow(magnitude(origin[0,0,:,0]).float().cpu(),cmap='jet')
    plt.title('t = 0')
    plt.subplot(2,3,2)
    plt.imshow(magnitude(origin[0,origin.shape[1]//2,:,0]).float().cpu(),cmap='jet')
    plt.title('t = '+str(origin.shape[1]//2))
    plt.subplot(2,3,3)
    plt.imshow(magnitude(origin[0,-1,:,0]).float().cpu(),cmap='jet')
    plt.title('t = '+str(origin.shape[1]-1))
    plt.subplot(2,3,4)
    plt.imshow(magnitude(pred[0,0,:,0]).float().cpu(),cmap='jet')
    plt.title('t = 0')
    plt.subplot(2,3,5)
    plt.imshow(magnitude(pred[0,pred.shape[1]//2,:,0]).float().cpu(),cmap='jet')
    plt.title('t = '+str(origin.shape[1]//2))
    plt.subplot(2,3,6)
    plt.imshow(magnitude(pred[0,-1,:,0]).float().cpu(),cmap='jet')
    plt.title('t = '+str(origin.shape[1]-1))
    plt.ioff()
